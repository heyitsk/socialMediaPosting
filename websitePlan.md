...
### 2.3 Threads (Threads API — separate Meta surface from Graph API) — **implemented Sep 21, 2026**
...
* **`threads-poster`**: 2-Step Flow — `POST https://graph.threads.net/v1.0/me/threads` (create container) ➔ **Wait Node** (5s text / 10s image / 60s video) ➔ `POST .../me/threads_publish`. Wired to the backend Sep 21, 2026 (§2.3); callback nodes gained the `x-callback-secret` header the same day, matching `facebook-poster`/`instagram-poster`. Known limitation carried over from the workflow's own sticky note: publish uses a blind time-based Wait rather than polling the container's `status` field until `FINISHED` — revisit if video publishes start failing with "media not ready" errors.
